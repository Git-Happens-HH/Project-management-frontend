...
const Dialog: React.FC<DialogProps> = ({ isOpen, toggleDialog, mode }) => {
   ...
   const isPasswordValid = /^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).*$/.test(userData.passwordHash);
   ...
   const handleSubmit = async (e: SubmitEvent) => {
      ...
      try {
         if (mode == 'login') {
            const token = await loginHandler(userData.email, userData.passwordHash)
            if (token) {
   ...
   const [formMode, setFormMode] = useState<typeof mode>(mode)
            ...
            <form onSubmit={handleSubmit} className="relative flex flex-col bg-white">
               ...
               <div className="flex flex-col gap-4 p-6">
                  ...
                  <div className="w-full max-w-sm min-w-[50]">
                     ...
                     />
                     {!isEmailValid && (<p className="text-red-500 text-sm">Email is missing @ or .</p>)}
                  </div>
                  ...
                  <div className="w-full max-w-sm min-w-[50]">
                     ...
                     />
                     {!isPasswordValid && (<p className="text-red-500 text-sm">Password has to have one Uppercase Letter, Special Character and Number</p>)}
                  </div>
               </div>
               ...
               <div className="p-6 pt-0">
                  ...
                  <p className="flex justify-center mt-6 text-sm text-slate-600">
                     ...
                     <button
                        type="button"
                        onClick={() => setFormMode(isLogin ? "register" : "login")}
               ...
               { showErrorMessage && (<div className="text-black text-center p-3 bg-amber-300"><p>{isLogin ? "Invalid credentials" : "An account already exists with this email address"}</p></div>)}
            </form>
         </div>
      </div>
   );
