...
function ProjectPage() {
   ...
   const promoteMember = async (member: ProjectMember) => {
      const token = localStorage.getItem("token");
      if (!token || !id) return;
      const username = `${member.username ?? ""}`;
      if (!confirm(`Do you want to promote ${username} to Owner (This will demote you to member)`)) return;
      try {
         await promoteUserToOwner(id, member.appUserId);
         const updatedMembers = await getProjectMembers(id);
         setMemberList(updatedMembers);
      } catch (error) {
         alert("Could not promote member to owner")
      }
   }
         ...
         >
            ...
            <div className="relative">
               <button
                  ...
                  className="border-2 rounded-full w-14 h-14 px-3 bg-white shadow-sm hover:shadow-md transition-shadow duration-200 hover:bg-(--prokress-orange) hover:border-(--prokress-orange)"
               >
                  <svg
                     xmlns="http://www.w3.org/2000/svg"
                     height="24px"
                                    ...
                                    <div>
                                       <Menu>
                                          ...
                                          <Menu.Content className="bg-(--prokress-beige-0) shadow-2xl shadow-black z-50 w-15 p-0.5">
                                             <Menu.Item className="h-5 justify-center p-4 hover:bg-(--prokress-orange)" onClick={() => promoteMember(member)}>
                                             ...
                                             <Menu.Item className="h-5 justify-center p-4 hover:bg-red-500" onClick={() => deleteMember(member)}><svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                height="30px"
                                                viewBox="0 -960 960 960"
                                                width="30px"
