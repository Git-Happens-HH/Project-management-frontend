...
function ProjectPage() {
    ...
    const newTask = ( taskListId: number) => {
        const token = localStorage.getItem("token");
        let taskTitle = prompt("New Task title", "New task title");
        let description = prompt("New description", "New description")
        if (token && id && taskTitle && description && taskListId) {
    ...
    const openProject = async (projectId: string) => {
        ...
        const res = await fetch(`https://project-management-backend-prokress-backend.2.rahtiapp.fi/api/projects/${projectId}/tasklists`, {
            headers: {
        ...
        if (!stompClient.current) {
            const socket = new SockJS("https://project-management-backend-prokress-backend.2.rahtiapp.fi/ws");
    ...
    function subscribeToProject(projectId: string) {
        ...
        subscription.current = stompClient.current.subscribe(
            ...
            (msg: IMessage) => {
                const transformed: TaskListsState = transformTaskLists(JSON.parse(msg.body));
                setTaskLists(transformed);
            }
        );
    }
    ...
    return (
        <div className="flex flex-col min-h-screen items-center bg-(--prokress-beige-100)">
            <div id="projectButtons" className="flex flex-row h-20 w-[75%] p-2 items-center justify-between">
                <div className="flex frex-row gap-2">
                    ...
                    <button onClick={newTaskList} className='w-20 bg-(--prokress-violet) hover:bg-(--prokress-orange) text-white font-bold py-2 px-4 rounded'
                    >Add</button>
                </div>
            ...
            <div id="taskListContainer" className="flex min-h-[70%] w-[76%] gap-2 p-3 overflow-auto bg-(--prokress-beige-50) rounded-2xl shadow">
                {Object.entries(taskLists).map(([column, taskList]) => (
                    <TaskList key={column} id={column} >
                        ...
                        <div onClick={() => {deleteTaskList(taskList.taskListId)}} className=" text-white font-bold py-2 px-2 rounded-full w-10 h-10 bg-(--prokress-black-500) hover:bg-red-500">
                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3"><path d="m376-300 104-104 104 104 56-56-104-104 104-104-56-56-104 104-104-104-56 56 104 104-104 104 56 56Zm-96 180q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520Zm-400 0v520-520Z" /></svg>
                        </div>
                        <div onClick={() => {newTask(taskList.taskListId)}} className=" text-white font-bold py-2 px-2 rounded-full w-10 h-10 bg-(--prokress-black-500) hover:bg-red-500">
                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#fff"><path d="M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z"/></svg>                        </div>
                        </div>
